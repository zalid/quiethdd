...
/*
* Modify Code for Purebasic because all parameters onlly long or pointer
* From ByValue to ByRef follows parameters: double, float, date
* New Indentifier Types: %i signed short, %I unsigned short, %r single (float)
*/
...
static HRESULT ExtractArgument(VARIANT * pvArg, WCHAR chIdentifier, BOOL * pbFreeArg, va_list * marker)
{
	...
	*pbFreeArg = FALSE;
	...
	switch (chIdentifier)
	{
		...
		case L'r':
			V_VT(pvArg)  = VT_R4;
			V_R4(pvArg)  = *va_arg(*marker, FLOAT *);
			break;
